...
def main():
    ...
    num_class, args.train_list, args.valid_list, args.root_path, prefix = dataset_config.return_dataset(args.dataset, args.modality)
    base_model = args.base_model
    args.store_name = '_'.join(['TDN_', args.dataset, args.modality, base_model, args.consensus_type, 'segment%d' % args.num_segments, 'e{}'.format(args.epochs)])
    if args.pretrain != 'imagenet':
        args.store_nmae += '_{}'.format(args.pretrain)
    ...
    for group in policies:
        logger.info(
            ('[TDN-{}]group: {} has {} params, lr_mult: {}, decay_mult: {}'.
             format(args.base_model, group['name'], len(group['params']),
